...
function Icon({ name }: { name: 'search' | 'user' | 'cart' | 'delivery' | 'check' | 'return' | 'heart' }) {
  const paths = {
    search: <><circle cx="11" cy="11" r="6.5" /><path d="m16 16 4 4" /></>,
    user: <><circle cx="12" cy="8" r="3.2" /><path d="M5.5 20c.7-3.4 2.9-5.1 6.5-5.1s5.8 1.7 6.5 5.1" /></>,
    cart: <><path d="M3 4h2l2 10.5h9.8l2-7.5H6" /><circle cx="9" cy="19" r="1" /><circle cx="17" cy="19" r="1" /></>,
    delivery: <><path d="M3 6h11v10H3z" /><path d="M14 10h3l3 3v3h-6z" /><circle cx="7" cy="18" r="1.7" /><circle cx="17" cy="18" r="1.7" /></>,
    check: <path d="m5 12 4 4 10-10" />,
    return: <><path d="M8 7H4v4" /><path d="M4 11a8 8 0 1 0 2-5" /></>,
    heart: <path d="M20 8.5c0 5-8 9.5-8 9.5S4 13.5 4 8.5A4.5 4.5 0 0 1 12 6a4.5 4.5 0 0 1 8 2.5Z" />,
  };
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{paths[name]}</svg>;
}
  ...
  return (
    <main>
      ...
      <section className="benefits" id="beneficios" aria-label="Benefícios da Loja">
        <div><span><Icon name="delivery" /></span><div><strong>Entrega rápida</strong><p>Receba sem complicação</p></div></div>
        <div><span><Icon name="check" /></span><div><strong>Compra segura</strong><p>Seus dados protegidos</p></div></div>
        <div><span><Icon name="return" /></span><div><strong>Troca fácil</strong><p>Sem burocracia</p></div></div>
        <div><span><Icon name="heart" /></span><div><strong>Atendimento humano</strong><p>Fale com a gente</p></div></div>
      </section>

      <section className="catalog" id="produtos" aria-labelledby="catalog-title">
        <div className="section-heading"><div><p className="eyebrow">SELEÇÃO DA SEMANA</p><h2 id="catalog-title">Produtos em destaque</h2></div><p className="result-count" aria-live="polite">{filtered.length} {filtered.length === 1 ? 'produto' : 'produtos'}</p></div>
        ...
        {filtered.length === 0 ? <div className="empty-state" role="status"><div aria-hidden="true"><Icon name="search" /></div><h3>Nenhum produto encontrado</h3><p>Tente outra busca ou categoria.</p>{(query || category !== 'Todos') && <button type="button" className="primary-btn" onClick={() => { setQuery(''); setCategory('Todos'); }}>Limpar filtros</button>}</div> : <div className="product-grid">{filtered.map((product, index) => {
          const out = product.stock <= 0;
          const isDemo = product.id.startsWith('demo-');
          const productHref = isDemo ? '#produtos' : `/produto/${product.id}`;
          const accents = ['paper', 'sand', 'stone', 'clay', 'ink'];
          return <article className="product-card" key={product.id}>
            <a href={productHref} className={`product-image ${accents[index % accents.length]}`} aria-label={`Ver detalhes de ${product.name}`}>
              ...
              <span className="category-pill">{categoryName(product.categories)}</span>{index < 2 && <span className="new-pill">NOVO</span>}
            </a>
            <div className="product-info"><p className="product-category">{categoryName(product.categories)}</p><a href={productHref}><h3>{product.name}</h3></a><p className="product-description">{product.description || 'Uma escolha especial para você.'}</p><div className="product-bottom"><div><strong>{money(product.price)}</strong><small>{out ? 'Esgotado' : `${product.stock} disponíveis`}</small></div><a className={out ? 'buy-btn disabled' : 'buy-btn'} href={productHref} aria-disabled={out}>{out ? 'Esgotado' : 'Comprar'}</a></div></div>
          </article>;
        })}</div>}
      </section>
