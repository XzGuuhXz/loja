...
as $$
declare current_row public.security_rate_limits%rowtype; now_ts timestamptz := clock_timestamp();
begin
  if p_key is null or length(p_key) < 8 or p_limit < 1 or p_window_seconds < 1 then return false; end if;
  select * into current_row from public.security_rate_limits where key = p_key for update;
  if not found then
    insert into public.security_rate_limits(key, window_started_at, hit_count, updated_at) values (p_key, now_ts, 1, now_ts) on conflict (key) do nothing;
    return true;
  end if;
